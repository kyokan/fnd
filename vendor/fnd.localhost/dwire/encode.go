...
// EncodeFields encodes each field in the variadic items argument
// into the Writer using the default Encoder.
func EncodeFields(w io.Writer, items ...interface{}) error {
	return defaultEncoder.EncodeFields(w, items...)
}

// EncodeField encodes a single field into the Writer using the
// default Encoder.
